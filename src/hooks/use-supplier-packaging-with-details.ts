...
export function useSupplierPackagingWithDetails() {
    const data = useLiveQuery(async () => {
        ...
        // Join data in memory
        const enriched: SupplierPackagingWithDetails[] = supplierPackaging.map((sp) => {
            const packaging = sp.packagingId ? packagingMap.get(sp.packagingId) : undefined;
...
export function useSupplierPackagingWithDetailsById(id: string | undefined) {
    const data = useLiveQuery(async () => {
        ...
        const [packaging, supplier] = await Promise.all([
            supplierPackaging.packagingId ? db.packaging.get(supplierPackaging.packagingId) : null,
