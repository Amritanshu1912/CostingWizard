 ...
 */
function convertFromStandard(quantityStandard: number, targetUnit: CapacityUnit): number {
...
export function useEnrichedRecipes(): RecipeDisplay[] {
    ...
    const enrichedRecipes = useMemo(() => {
        ...
        return recipes.map((recipe): RecipeDisplay => {
                    ...
                    return {
                        ...
                        priceChangedSinceLock: ing.lockedPricing
                            ? sm
                                ...
                                : false
                            : false,
                        priceDifference: ing.lockedPricing && sm
                            ? sm.unitPrice - ing.lockedPricing.unitPrice
                            : undefined,
            ...
            const totalWeight = recipe.totalWeight || 0; // Use recipe's totalWeight
            ...
            const costPerKg = totalWeight > 0 ? totalCost / normalizeToKg(totalWeight, "gm") : 0;
...
export function useRecipeVariants(
    ...
    const variants = useMemo(() => {
        ...
        return data.variants
            ...
            .map((variant) => {
                // For now, calculate cost based on original recipe
                // In a full implementation, this would apply the changes to get the variant's actual cost
                const variantCostPerKg = originalRecipe.costPerKg; // Placeholder
                const costDifference = variantCostPerKg - originalRecipe.costPerKg;
                const costDifferencePercentage = originalRecipe.costPerKg > 0
...
export function useRecipeVariantSummary(recipeId: string | null | undefined) {
    ...
    const summary = useMemo(() => {
        ...
        const costOptimizedVariants = variants.filter(v =>
            v.optimizationGoal === "cost_reduction"
        ).length;
...
export const recipeCalculator = {
    ...
    calculateRecipeMetrics(
        ...
        const taxedCostPerKg =
            totalWeight > 0
                ? taxedTotalCost / normalizeToKg(totalWeight, "gm")
                : 0;
